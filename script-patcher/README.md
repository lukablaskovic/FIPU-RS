# PDF i Google Drive — RS

Iz korijena repozitorija:

```bash
make setup                  # Jednom: lokalne ovisnosti i hook za datum
make pdf SCRIPT=RS1        # Samo lokalni PDF
make upload SCRIPT=RS1     # Markdown i postojeći PDF na Drive
make sync SCRIPT=RS1       # PDF pa prijenos, samo ako izvoz uspije
```

Odabir: `RS1`–`RS8`. Potrebni su Node.js 22+, Python 3 i Pandoc (`brew install pandoc` na macOS-u). `make` prikazuje pomoć. Ručne naredbe ne rade commit/push.

Odredište: [Raspodijeljeni sustavi](https://drive.google.com/drive/folders/1hzmr0AfV9R2azSC76n1Ryai0X4FDaTOc). Odabiru se samo 8 glavnih Markdown/PDF parova. Primjeri aplikacija, ispiti i dodatni zadaci ne prenose se.
...
Provjere: `python3 script-patcher/test_sync.py`, `python3 .githooks/test_dates.py`, `npm test --prefix script-patcher`. Prikaz odabira: `python3 script-patcher/index.py --list-local`. Plan prijenosa bez pisanja: `script-patcher/.venv/bin/python script-patcher/index.py --script RS1 --dry-run`.
